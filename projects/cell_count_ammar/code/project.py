...
import os
import numpy as np
from PyQt5.QtWidgets import QApplication, QDialog, QFileDialog # library for GUI
from gui import Ui_Form # GUI File imported
...
class MainWindow(QDialog, Ui_Form):
    def __init__(self, parent=None):
        ...
        # these functions are also called event functions
        ...
        # connecting spin boxes with function
    ...
    def get_file_path(self): # getting file path
        dir_choose = QFileDialog.getExistingDirectory(self, "Select Folder", os.getcwd()) # Open button folder
        temp_list = os.listdir(dir_choose)
        for i in temp_list:
            self.path_list.append(dir_choose + "/" + i)
        self.path_list_i = 0
        self.shulaibao()# Opencv function

    def press_next(self): # next button
    ...
    def press_pre(self): # previous button
    ...
    def change_min_grayscale_slider(self): # function associated with slider
    ...
    def change_max_grayscale_slider(self):# function associated with slider
    ...
    def change_center_x(self): # funciton for testbox/spin box center_x
        self.CENTER_X = self.center_x_spin.value()

        self.shulaibao()

    def change_center_y(self):# funciton for testbox/spin box center_y
        self.CENTER_Y = self.center_y_spin.value()
        self.shulaibao()

    def change_r(self):# funciton for testbox/spin box radius
        self.RADIUS = self.r_spin.value()
        self.shulaibao()

    def change_left(self):# funciton for testbox/spin box Left
        self.left_line = self.left_spin.value()
        self.shulaibao()

    def change_right(self):# funciton for testbox/spin box right
        self.right_line = self.right_spin.value()
        self.shulaibao()

    def change_down(self):# funciton for testbox/spin box Down
        self.right_line = self.right_spin.value()
        self.shulaibao()

    def change_up(self): # funciton for testbox/spin box Up
        self.right_line = self.right_spin.value()
        self.shulaibao() 

    def bias_change(self): # nine spin boxes
    ...
    def shulaibao(self): # function for OpenCV stuff
